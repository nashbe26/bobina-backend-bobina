...
const register = async (data) =>{
  ...
  if (!(data && data.password && data.email &&  data.username  &&  data.name))
	...
	// Verfiy username
	if (data.username.length > 20 || !data.username.match(onlyAlphabetRegex))
		throw createError(400, 'Bad format username');

	// Verfiy name
	if (data.name.length > 20 || !data.name.match(onlyAlphabetRegex))
		throw createError(400, 'Bad format name');
